package at.ac.uibk.dps.streamprocessingapplications.train.tasks;
...
public abstract class AbstractTask<T, U> implements ITask<T, U> {
   ...
   * indicating the status or value of the execution. Float.MIN_FLOAT means there was an error. All
   * other values are valid. Null return means there is no output to pass on.
   */
  @Override
  ...
  /**
   * Returns the last result that was set by the child task in case there is a result other than the
   * float returned by doTaskLogic()
   *
   * @return
   */
  ...
  /**
   * Set by child task if there are results other than the float result that need to be accessed by
   * invoker.
   *
   * @param r
   */
